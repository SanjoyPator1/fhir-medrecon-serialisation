...
import { MODEL_SHORT } from "@/lib/constants"
import type { ExperimentRow } from "@/lib/data"
import type { GroundTruth } from "@/lib/data"

interface PatientStrategyRow {
  patient_id: string
  patient_name: string
  active_med_count: number
  mean_f1: number
  models: Record<string, { f1: number; parse_failed: boolean } | undefined>
}

interface StrategyPatientTableProps {
  rows: ExperimentRow[]
  models: string[]
...
export function StrategyPatientTable({ rows, models, groundTruth }: StrategyPatientTableProps) {
  const [query, setQuery] = useState("")
  const [sortKey, setSortKey] = useState<SortKey>("mean_f1")
  const [sortDir, setSortDir] = useState<SortDir>("asc")
  const [page, setPage] = useState(0)

  const tableRows = useMemo<PatientStrategyRow[]>(() => {
    ...
    const result: PatientStrategyRow[] = []
    for (const [patient_id, patRows] of byPatient) {
      ...
      const mean_f1 = patRows.length
        ...
        : 0
      const modelMap: PatientStrategyRow["models"] = {}
      for (const r of patRows) {
        modelMap[r.model] = { f1: r.f1, parse_failed: r.parse_failed }
      }
      result.push({
        patient_id,
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent border-b">
              ...
              {models.map((m) => (
                <TableHead key={m} className="text-xs uppercase text-muted-foreground font-medium text-center min-w-[80px]">
                  {MODEL_SHORT[m] ?? m}
                </TableHead>
              ))}
            </TableRow>
          ...
          <TableBody>
            {pageItems.length === 0 && (
              <TableRow>
                <TableCell colSpan={3 + models.length} className="text-center text-muted-foreground py-8 text-sm">
                  No patients found
                </TableCell>
              </TableRow>
            )}
            {pageItems.map((p) => {
              const hasFailure = models.some((m) => p.models[m]?.parse_failed)
              return (
                <TableRow
                  key={p.patient_id}
                  className={cn(
                ...
                >
                  ...
                  {models.map((m) => (
                    <TableCell key={m} className="py-2 text-center">
                      {f1Cell(p.models[m])}
                    </TableCell>
                  ))}
                </TableRow>
