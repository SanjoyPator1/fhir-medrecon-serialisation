...
const DASHBOARD_SECTIONS = [
  ...
  },
  {
    ...
    href: "/patients",
    desc: "The core inspection view. A model × strategy grid shows F1, precision, recall, and inference time for every run on this patient. Click any cell to load a side-by-side diff of the ground truth medications versus the model output — green for matched, red for missed or hallucinated.",
  },
]
  ...
  return (
    <div className="space-y-10 max-w-4xl">
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-2">
          {DASHBOARD_SECTIONS.map(({ page, href, desc }) => (
            <div key={page} className="flex gap-3">
              <Link
                href={href}
                className="shrink-0 font-mono text-xs font-semibold bg-muted rounded px-1.5 py-0.5 h-fit mt-0.5 whitespace-nowrap hover:bg-muted/70 transition-colors"
              >
                {page}
              </Link>
